...
XrmServiceToolkit.Common = function () {
    ...
    var alertMessage = function (message) {
        (window.parent.Xrm.Utility !== undefined && window.parent.Xrm.Utility.alertDialog !== undefined) ? window.parent.Xrm.Utility.alertDialog(message) : alert(message);
    };
    ...
    var updateRequirementLevel = function (fieldName, levelName) {
        ...
        /// </param>
        /// <returns type="void" />
        window.parent.Xrm.Page.getAttribute(fieldName).setRequiredLevel(levelName);
    };
    ...
    var addNotification = function (message, level, uniqueId) {
        ...
        if (window.parent.Xrm.Page.ui.setFormNotification !== undefined) {
            // CRM 2013
            if (!!uniqueId) {
                window.parent.Xrm.Page.ui.clearFormNotification(uniqueId);
                if (level === 1) {
                    //Error
                    window.parent.Xrm.Page.ui.setFormNotification(message, "ERROR", uniqueId);
                }
                if (level === 2) {
                    //Info
                    window.parent.Xrm.Page.ui.setFormNotification(message, "INFO", uniqueId);
                }
                if (level === 3) {
                    //Warning
                    window.parent.Xrm.Page.ui.setFormNotification(message, "WARNING", uniqueId);
                }
            } else {
                var tempUniqueId = "formNotification00";
                window.parent.Xrm.Page.ui.clearFormNotification(tempUniqueId);
                if (level === 1) {
                    //Error
                    window.parent.Xrm.Page.ui.setFormNotification(message, "ERROR", tempUniqueId);
                }
                if (level === 2) {
                    //Info
                    window.parent.Xrm.Page.ui.setFormNotification(message, "INFO", tempUniqueId);
                }
                if (level === 3) {
                    //Warning
                    window.parent.Xrm.Page.ui.setFormNotification(message, "WARNING", tempUniqueId);
                }
            }
        }
        else {
    ...
    var addControlNotification = function (attributeName, message) {
        if (window.parent.Xrm.Page.getControl(attributeName).setNotification !== undefined) {
            window.parent.Xrm.Page.getControl(attributeName).setNotification(message);
        }
    };
    ...
    var disableAllControlsInTab = function (tabControlNo) {
        ...
        /// </param>
        /// <returns type="void" />
        var tabControl = window.parent.Xrm.Page.ui.tabs.get(tabControlNo);
        if (tabControl != null) {
            window.parent.Xrm.Page.ui.controls.forEach(
             function (control) {
...
XrmServiceToolkit.Soap = function () {
                    ...
                    if (attribute === null || attribute.value === null) {
                        ...
                        case "EntityCollection":
                            ...
                            for (var i = 0, collectionLengh = collections.length; i < collectionLengh; i++) {
                                ...
                                if (window.parent.Xrm.Utility.openQuickCreate !== undefined) {
    ...
    var fetch = function (fetchCore, fetchAll, callback, errorCallback) {
        ...
        var fetchXml = fetchCore.replace(/\"/g, "'");
        ...
        //    // BUG FIX
        //    var isLimitedReturn = ((fetchCore.indexOf("page=\"1\"") !== -1) && (fetchCore.indexOf("count=") !== -1 || fetchCore.indexOf("count='") !== -1)) ||
        //        (fetchCore.indexOf("page='1'") !== -1 && fetchCore.indexOf("count='") !== -1);
